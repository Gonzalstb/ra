...
class SyncTripsRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'trips.*.routeSegments.*.toKey' => 'nullable|string',
            'trips.*.routeSegments.*.sameRoadAs' => 'nullable|string',
            'trips.*.routeSegments.*.dayId' => 'nullable|string',
            'trips.*.routeSegments.*.travelDate' => 'nullable|date_format:Y-m-d',
            'trips.*.routeSegments.*.lineColor' => 'nullable|string|regex:/^#[0-9A-Fa-f]{6}$/',
            'trips.*.routePlans' => 'present|array',
            'trips.*.routePlans.*.id' => 'required|string',
            'trips.*.routePlans.*.name' => 'required|string|max:80',
            'trips.*.routePlans.*.segments' => 'present|array',
            'trips.*.routePlans.*.segments.*.id' => 'required|string',
            'trips.*.routePlans.*.segments.*.fromKey' => 'required|string',
            'trips.*.routePlans.*.segments.*.toKey' => 'nullable|string',
            'trips.*.routePlans.*.segments.*.sameRoadAs' => 'nullable|string',
            'trips.*.routePlans.*.segments.*.dayId' => 'nullable|string',
            'trips.*.routePlans.*.segments.*.travelDate' => 'nullable|date_format:Y-m-d',
            'trips.*.routePlans.*.segments.*.lineColor' => 'nullable|string|regex:/^#[0-9A-Fa-f]{6}$/',
            'trips.*.activeRoutePlanId' => 'nullable|string',
